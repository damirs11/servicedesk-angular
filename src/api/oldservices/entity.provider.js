  ...
  return class Entity {
    ...
    $serialize() {
      ...
      for (const key in this) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
          // Создаем нужные объекты по цепочке
      ...
      json[this.constructor.$tracker.dataField] = this[
        this.constructor.$tracker.ownField
      ];
      return json;
    }
    ...
    get $modifiedData() {
      ...
      for (let key in this.$data) {
        ...
        if ("$modifiedData" in Object(val) && val.checkModified())
          fields.push(key);
      }
