 ...
 */
@Component
public class AssignmentSimpleMapper extends EntityRowMapper<Assignment> {
    ...
    public Assignment mapRow(ResultSet rs, int rowNumber) throws SQLException {
        ...
        if (executorId != null) {
            assignment.setExecutor(new Person(executorId));
        }

        Long workgroupId = DBUtils.getLong(rs, "ass_workgroup_to");
        if (workgroupId != null) {
            assignment.setWorkgroup(new Workgroup(workgroupId));
        }
        return assignment;
    }
}
