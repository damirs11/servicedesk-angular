...
public class AppLoggingInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		LOG.debug(ResourceMessages.getMessage("http.request.start"));
		return true;
	}
	...
	private void LogComplete(HttpServletResponse response) {
		LOG.debug(ResourceMessages.getMessage("http.request.result", response.getStatus(), HttpStatus.valueOf(response.getStatus()).name()));
	}
}